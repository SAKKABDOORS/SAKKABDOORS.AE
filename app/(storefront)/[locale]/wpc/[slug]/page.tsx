...
export async function generateMetadata({
  ...
  if (!product || product.material !== "WPC") return {};
  ...
  return {
    title: name,
    description,
    alternates: { languages: { ar: `/ar/wpc/${params.slug}`, en: `/en/wpc/${params.slug}` } },
    openGraph: { title: name, description, images: image ? [image] : undefined }
  };
}

export default async function WpcProductPage({
  params
}: {
  params: { locale: string; slug: string };
}) {
  ...
  if (!product || product.material !== "WPC") notFound();
